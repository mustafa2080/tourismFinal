...
export function AuditLogsPage() {
  ...
  const getActionColor = (action) => {
    if (action?.includes('BAN') || action?.includes('REJECT') || action?.includes('DELETE')) {
      ...
      return 'bg-amber-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-500 border-amber-300 dark:border-teal-700';
    }
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-3xl w-full flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in scale-95 duration-200">
            ...
            <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 overflow-y-auto">
              ...
              {/* Admin Information */}
              <div className="bg-[#0d9488] rounded-xl p-6 border border-amber-200 dark:border-teal-800">
                <p className="text-xs font-bold text-teal-600 dark:text-teal-500 uppercase tracking-wide mb-3">Admin Information</p>
              ...
              {/* Date & Time */}
              <div className="bg-[#0d9488] rounded-xl p-6 border border-green-200 dark:border-green-800">
            ...
            <div className="p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700">
              <button
                onClick={() => setShowModal(false)}
                className="w-full px-6 py-3 rounded-lg bg-[#0d9488] text-white transition-all font-bold shadow-lg hover:shadow-xl"
              >
                Close
              </button>
            </div>
